...
$brands_query = "SELECT DISTINCT brand FROM Product WHERE categoryID = 6 AND status = 'Active' ORDER BY brand";
$brands_result = mysqli_query($conn, $brands_query);

// Get unique storage type for filter
$com_type_query = "SELECT DISTINCT ps.attributeValue AS Usetype 
                   FROM Product_Specification ps 
                   JOIN Product p ON ps.productID = p.productID 
                   WHERE p.categoryID = 6 AND p.status = 'Active' AND ps.attributeName = 'useType' 
                   ORDER BY Usetype";
$com_type_result = mysqli_query($conn, $com_type_query);

// Get unique capacities for filter
$capacity_query = "SELECT DISTINCT ps.attributeValue AS capacity 
                   FROM Product_Specification ps 
                   JOIN Product p ON ps.productID = p.productID 
                   WHERE p.categoryID = 6 AND p.status = 'Active' AND ps.attributeName = 'storage' 
                   ORDER BY capacity";
...
<body>
    ...
    <!-- Storage Section -->
    <section class="products-listing" data-category="storage" data-category-label="Storage Devices">
        <div class="container">
                ...
                <aside class="filters-sidebar">
                    <form method="GET" action="components.php" id="filter-form">
                        ...
                        <div class="filter-group">
                            <h4>Capacity</h4>
                            <div class="filter-options">
                                <?php while($cap = mysqli_fetch_array($capacity_result)): ?>
                                    <?php if(!empty($cap['capacity'])): ?>
                                    <label class="filter-option">
                                        <input type="checkbox" name="storage[]" value="<?php echo htmlspecialchars($cap['capacity']); ?>"
                                            <?php echo (in_array($cap['capacity'], (array)$storage_filter)) ? 'checked' : ''; ?>>
                                        <span class="checkmark"></span>
                ...
                <main class="products-main">
                    <div class="products-header">
                        <div class="results-info">
                            <p>Showing <strong id="total-count-display">0</strong> Storage Devices</p>
                        </div>
